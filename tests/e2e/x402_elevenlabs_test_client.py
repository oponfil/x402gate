"""E2E test client for x402gate → ElevenLabs TTS.
...
Usage:
    python tests/e2e/x402_elevenlabs_test_client.py
"""
...
logger = logging.getLogger("x402-elevenlabs-client")
...
def _load_examples() -> list[tuple[str, dict, str]]:
    ...
    provider = cfg["providers"]["elevenlabs"]
    examples = []
    for key in ("example_request", "example_request_2"):
        if key in provider:
            ex = provider[key]
            label = ex["body"].get("model_id", "default")
            examples.append((ex["model"], dict(ex["body"]), label))
    return examples
...
async def _run_single(
    ...
    logger.info("Voice ID: %s, Text: %s", model, body.get("text", "?")[:80])

    timings = Timings()

    # 1. Request without payment → expect 402
    logger.info("Sending initial request to ElevenLabs via gateway...")
    with timings.measure("pricing"):
        response = await http_client.post(
            f"{gateway_url}/v1/elevenlabs/{model}",
            json=body,
            timeout=15.0,
        )
    ...
    if audio_b64:
        saved = save_audio(audio_b64, f"elevenlabs_{label}", content_type)
...
async def run_client():
    """Run the E2E client test for ElevenLabs TTS (both examples)."""
