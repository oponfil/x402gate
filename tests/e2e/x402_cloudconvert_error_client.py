"""E2E error client: sends a paid request with an unsupported operation.

The gateway should verify payment, call CloudConvert submit(),
get a validation error (bad operation), and return the error
WITHOUT settling payment — client keeps USDC.

Usage:
    python tests/e2e/x402_cloudconvert_error_client.py
"""
...
logger = logging.getLogger("x402-cloudconvert-error-client")

# A small HTML file (same as the happy-path test)
TEST_FILE_CONTENT = b"""<!DOCTYPE html>
<html>
<head><title>Error Test</title></head>
<body><p>This request should fail.</p></body>
</html>"""

TEST_FILE_NAME = "test_error.html"
...
async def run_client():
    ...
    async with httpx.AsyncClient() as http_client:
        # 1. Get 402 with valid request (need this for pricing)
        logger.info("Getting 402 with valid request for pricing...")
        response = await http_client.post(
            url,
            files={"file": (TEST_FILE_NAME, TEST_FILE_CONTENT)},
            data={"output_format": "pdf", "input_format": "html"},
        )
        ...
        # 3. Send paid request with INVALID operation → should get error
        logger.info("Sending paid request with unsupported operation...")
        response = await http_client.post(
            url,
            files={"file": (TEST_FILE_NAME, TEST_FILE_CONTENT)},
