"""E2E test: invalid URL → SocialDownload error → no settlement → client keeps money."""
...
@pytest.mark.asyncio
async def test_socialdownload_bad_url_no_settlement(gateway_process, base_chain):
    """Send invalid URL to SocialDownload, verify error + no money lost."""
    ...
    result = run_script(
        "tests/e2e/x402_socialdownload_error_client.py", label="SocialDownload Error"
    )
