"""E2E test client for x402gate → Fish Audio TTS.
...
Usage:
    python tests/e2e/x402_fishaudio_test_client.py
"""
...
logger = logging.getLogger("x402-fishaudio-client")
...
def _load_examples() -> list[tuple[str, dict, str]]:
    ...
    provider = cfg["providers"]["fishaudio"]
    examples = []
    for key in ("example_request", "example_request_2"):
        if key in provider:
            ex = provider[key]
            lang = "ru" if any(ord(c) > 127 for c in ex["body"].get("text", "")) else "en"
            examples.append((ex["model"], dict(ex["body"]), f"{ex['model']}_{lang}"))
    return examples
...
async def _run_single(
    ...
    logger.info("Sending initial request to FishAudio via gateway...")
    with timings.measure("pricing"):
        response = await http_client.post(
            f"{gateway_url}/v1/fishaudio/{model}",
            json=body,
            timeout=15.0,
        )
    ...
    utf8_bytes = data.get("utf8_bytes", 0)

    if audio_b64:
        saved = save_audio(audio_b64, f"fishaudio_{label}", content_type)
        logger.info("Audio saved: %s (%d chars, %d UTF-8 bytes)", saved, chars, utf8_bytes)
    else:
...
async def run_client():
    """Run the E2E client test for Fish Audio TTS (both examples)."""
