"""E2E test client for x402gate -> OpenRouter Embeddings.

Simulates a user:
1. Sends an embedding request to the gateway.
2. Receives a 402 Payment Required response.
3. Signs the x402 payment (Exact EVM Scheme, Base).
4. Resends with payment signature -> gets embedding vector.

Usage:
    python tests/e2e/x402_embedding_test_client.py
"""
...
logger = logging.getLogger("x402-embedding-client")

CONFIG_PATH = Path(__file__).parent.parent.parent / "config.yaml"


def _load_example_request() -> tuple[str, dict]:
    """Load model and body from config.yaml's openrouter example_request_3 (embedding)."""
    with open(CONFIG_PATH, encoding="utf-8") as f:
        cfg = yaml.safe_load(f)
    ex = cfg["providers"]["openrouter"]["example_request_3"]
    return ex["model"], dict(ex["body"])


async def run_client():
    """Run the E2E client test for OpenRouter Embeddings."""
    ...
    logger.info("Input: %s", body.get("input", ""))
    ...
    async with httpx.AsyncClient() as http_client:
        ...
        logger.info("Sending initial embedding request via gateway...")
        with timings.measure("pricing"):
            response = await http_client.post(
                f"{gateway_url}/v1/openrouter/{model}",
                json=body,
                timeout=15.0,
            )
        ...
        # 3. Retry with payment -> expect 200 with embedding vector
        ...
        # Embedding response: data is a list of embedding objects
        embeddings = data if isinstance(data, list) else data.get("data", [])

        if not embeddings:
            logger.error("No embeddings in response: %s", result)
            sys.exit(1)

        first_embedding = embeddings[0]
        vector = first_embedding.get("embedding", [])
        usage = (
            result.get("usage", data.get("usage", {}))
            if isinstance(data, dict)
            else result.get("usage", {})
        )

        logger.info("Embedding model: %s", body.get("model", "?"))
        logger.info("Vector dimensions: %d", len(vector))
        logger.info("First 5 values: %s", vector[:5])
        logger.info(
            "Tokens: prompt=%s, total=%s",
            usage.get("prompt_tokens"),
            usage.get("total_tokens"),
        )
        logger.info("E2E embedding test passed!")
